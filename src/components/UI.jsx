...
export function Btn({ children, onClick, variant = 'primary', style, disabled }) {
  ...
  const v = { primary: { ...base, background: 'var(--red)', color: 'white', fontWeight: 700, fontSize: 15, padding: '14px 28px', letterSpacing: '0.02em' }, secondary: { ...base, background: 'var(--surface)', color: 'var(--text)', fontWeight: 600, fontSize: 14, padding: '12px 24px', border: '1px solid var(--border)' }, ghost: { ...base, background: 'transparent', color: 'var(--text-muted)', fontWeight: 500, fontSize: 13, padding: '8px 16px', width: 'auto' } };
...
export function Input({ value, onChange, placeholder, style, maxLength, type = 'text' }) {
  return <input value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} maxLength={maxLength || 30} type={type} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '14px 16px', color: 'var(--text)', fontSize: 15, width: '100%', transition: 'border-color 0.2s', outline: 'none', fontFamily: 'var(--font-body)', ...style }} onFocus={e => (e.target.style.borderColor = 'var(--red)')} onBlur={e => (e.target.style.borderColor = 'var(--border)')} />;
}
...
export function PlayerAvatar({ avatarUrl, emoji, size = 40, style }) {
  if (avatarUrl) return <img src={avatarUrl} alt="" style={{ width: size, height: size, borderRadius: '50%', objectFit: 'cover', border: '2px solid var(--border)', flexShrink: 0, ...style }} />;
...
export function PhotoUpload({ onCapture, previewUrl, onClear, label, required }) {
  const cameraRef = React.useRef(null);
  const fileRef = React.useRef(null);
  const handleFile = (e) => { const file = e.target.files?.[0]; if (file) onCapture(file); };
  if (previewUrl) return (
    <div style={{ position: 'relative' }}>
      <img src={previewUrl} alt="" style={{ width: '100%', maxHeight: 200, borderRadius: 12, objectFit: 'cover' }} />
      <button onClick={() => { onClear(); if (cameraRef.current) cameraRef.current.value = ''; if (fileRef.current) fileRef.current.value = ''; }} style={{ position: 'absolute', top: 8, right: 8, width: 28, height: 28, borderRadius: '50%', background: 'rgba(0,0,0,0.7)', border: 'none', color: 'white', fontSize: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
    </div>
  );
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <button onClick={() => cameraRef.current?.click()} style={{ flex: 1, padding: '12px 8px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, color: 'var(--text)', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--font-body)' }}>📷 Camera</button>
        <button onClick={() => fileRef.current?.click()} style={{ flex: 1, padding: '12px 8px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, color: 'var(--text)', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--font-body)' }}>🖼️ Gallery</button>
      </div>
